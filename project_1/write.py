...
def write_to_csv(results, filename):
    ...
    row corresponds to the information in a single close approach
    ...
    with open("close_approaches.csv", "w", newline='') as outfile:
...
def write_to_json(results, filename):
    ...
    the output is a list containing dictionaries, each
    mapping `CloseApproach` attributes to their values and the
    'neo' key mapping to a dictionary of the associated NEO's attributes.
    ...
    for result in results:
        ...
        data_to_write.append(
            dict(datetime_utc=datetime_to_str(row["datetime_utc"]),
                 distance_au=row["distance_au"],
                 velocity_km_s=row["velocity_km_s"],
                 neo=dict(designation=row["designation"],
                          name=row["name"], diameter_km=row["diameter_km"],
                          potentially_hazardous=bool(
                          row["potentially_hazardous"]))))

    with open(filename, "w") as file:
        json.dump(data_to_write, file, indent="\t")
